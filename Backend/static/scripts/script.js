...
;(function($) {
	...
	for (var n = 0; n < data.length; n++) {
		if (
			...
			html.addClass(
				data[n].name + ' ' + data[n].name + v(navigator.userAgent, n) ||
...
var ThemeLightbox = (function($) {
	...
	return function() {
		...
		function show(src) {
			var d = $(
				'<div id="art-lightbox-bg"><div class="close"><div class="cw"> </div><div class="ccw"> </div><div class="close-alt">&#10007;</div></div></div>',
			)

			var img = $(
				'<img class="art-lightbox-image active" alt="" src="' +
					getFullImgSrc($(src).attr('src')) +
					'" />',
			)
		...
		function showError(enable) {
			if (enable) {
				b.append(
					$(
						'<div class="lightbox-error">El contenido deseado no puede cargarse.<br/>Por favor inténtelo más tarde.</div>',
					).css({
		...
		var close = function() {
			$(
				'#art-lightbox-bg, .art-lightbox-image, .arrow, .lightbox-error',
			).remove()
		}
		...
		function resizeOnLoad(img) {
			...
			img.load(function() {
				...
				img.css({
					top: height / 2 - imgHeight / 2 - 5,
					left: width / 2 - imgWidth / 2 - 5,
				})
			})
		...
		function getFullImgSrc(src) {
			var webArchiveRegex = new RegExp(
				'http://www.[A-z0-9-]+-image.com/.webarchive/',
			)
			if (
				(src.indexOf('http://') === 0 || src.indexOf('https://') === 0) &&
				!webArchiveRegex.test(src)
			) {
				return src
			}
...
;(function($) {
	...
	window.BackgroundHelper = function() {
		...
		this.transition = function(container, on) {
			container.css(
				$.support.transition.prefix + 'transition',
				on ? transitionDuration + ' ease-in-out background-position' : '',
			)
		}
	...
	var Slider = function(element, settings) {
		...
		if (this.settings.clickevents) {
			$(this.settings.navigator).on('click', 'a', { slider: this }, function(
				event,
			) {
...
jQuery(function($) {
	...
	header.css(
		'background-image',
		"url('images/header.png')".replace(/(url\(['"]?)/i, '$1' + path),
	)
